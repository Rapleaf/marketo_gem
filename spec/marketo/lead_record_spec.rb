  ...
  module Marketo
    ...
    describe LeadRecord do
      it "should store the idnum" do
        lead_record = LeadRecord.new(TEST_EMAIL, TEST_IDNUM)
        lead_record.idnum.should == TEST_IDNUM
      end

      it "should store the email" do
        LeadRecord.new(TEST_EMAIL, TEST_IDNUM).email.should == TEST_EMAIL
      end

      it "should implement == sensibly" do
        lead_record1 = LeadRecord.new(TEST_EMAIL, TEST_IDNUM)
        lead_record1.set_attribute('favourite color', 'red')
        lead_record1.set_attribute('age', '100')

        lead_record2 = LeadRecord.new(TEST_EMAIL, TEST_IDNUM)
      ...
      it "should be instantiable from a savon hash" do
        ...
        expected = LeadRecord.new(TEST_EMAIL, TEST_IDNUM)
